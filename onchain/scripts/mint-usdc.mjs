...
const main = async () => {
  ...
  const amountArg = getArg('--amount') ?? '1'
  const isRaw = process.argv.includes('--raw')
  const amount = isRaw ? BigInt(amountArg) : ethers.parseUnits(amountArg, 6)
  ...
  await tx.wait()
  console.log(`Minted ${amount.toString()} (raw) to ${to} on ${tokenAddress}`)
}
